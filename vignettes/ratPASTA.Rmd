...
Alternatively, data can be loaded by specifying the names of all pasta files. To do so, set `auto_import` argument to `FALSE` and specify `data` as a list containing all pasta data. To do so, manualy import all .pasta files as csv files, place them in a list. Each element should have the same name as the csv. 

``` 
df <- loadStartleData(auto_import = FALSE, data = list)
```
